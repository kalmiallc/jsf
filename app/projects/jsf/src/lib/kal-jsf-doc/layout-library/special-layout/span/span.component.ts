...
import { JsfLayoutSpan, JsfSpecialLayoutBuilder, PropStatus, PropStatusChangeInterface } from '@kalmia/jsf-common-es2015';
import { AbstractSpecialLayoutComponent }                                                from '../../../abstract/special-layout.component';
import { BuilderDeveloperToolsInterface }                                                from '../../../builder-developer-tools.interface';
import { takeUntil }                                                                     from 'rxjs/operators';

@Component({
  selector       : 'jsf-layout-span',
  template       : `
      <span class="jsf-layout-span" [ngClass]="htmlClass" (click)="handleLayoutClick($event)">{{ title }}</span>
  `,
  changeDetection: ChangeDetectionStrategy.OnPush,
  styles         : []
})
export class LayoutSpanComponent extends AbstractSpecialLayoutComponent<JsfLayoutSpan> implements OnInit {
    ...
    if (this.layout.templateData) {
      if (this.dependencies.length) {
        ...
        if (this.layoutBuilder.rootBuilder.warnings) {
          console.warn(`Layout 'span' [${ this.layoutBuilder.id }] uses templateData but has not listed any dependencies.`,
