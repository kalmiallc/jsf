...
import { JsfLayoutSub, JsfSpecialLayoutBuilder, PropStatus, PropStatusChangeInterface } from '@kalmia/jsf-common-es2015';
import { AbstractSpecialLayoutComponent }                                               from '../../../abstract/special-layout.component';
import { BuilderDeveloperToolsInterface }                                               from '../../../builder-developer-tools.interface';
import { takeUntil }                                                                    from 'rxjs/operators';

@Component({
  selector       : 'jsf-layout-sub',
  template       : `
      <sub class="jsf-layout-sub" [ngClass]="htmlClass" (click)="handleLayoutClick($event)">{{ title }}</sub>
  `,
  changeDetection: ChangeDetectionStrategy.OnPush,
  styles         : []
})
export class LayoutSubComponent extends AbstractSpecialLayoutComponent<JsfLayoutSub> implements OnInit {
    ...
    if (this.layout.templateData) {
      if (this.dependencies.length) {
        ...
        if (this.layoutBuilder.rootBuilder.warnings) {
          console.warn(`Layout 'sub' [${ this.layoutBuilder.id }] uses templateData but has not listed any dependencies.`,
